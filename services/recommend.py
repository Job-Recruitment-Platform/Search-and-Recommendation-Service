   ...
   # ================== Recommendation Entrypoint ==================
    ...
    def _generate_cf_candidates(
        ...
        try:
            ...
            max_item_index = max(
                self.cf_index_to_item_id.keys()) if self.cf_index_to_item_id else -1
    ...
    def _generate_content_candidates(
        ...
        try:
            ...
            # hybrid_search requires rerank parameter, use WeightedRanker with
            # weight 1.0 for dense only
        ...
        if not search_results or len(
                search_results) == 0 or not search_results[0]:
            return []
    ...
    def _calculate_long_term_user_vector(
        ...
        """
        # Build profile text (without interaction insights for long-term
        # vector)
    ...
    def _process_interaction_stream(self):
        ...
        try:
            ...
            print(f"[STREAM] Read from '{stream_name}': {len(messages)} batch(es)")
    ...
    def _handle_interaction_event(self, fields: Dict[str, bytes]):
        ...
        try:
            ...
            # Extract fields
            account_id = event_data.get(
                "accountId") or event_data.get("account_id")
            job_id = event_data.get("jobId") or event_data.get("job_id")
            event_type = event_data.get(
                "eventType") or event_data.get("event_type")
            occurred_at = event_data.get(
                "occurredAt") or event_data.get("occurred_at")
