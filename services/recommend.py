...
class RecommendationService:
   ...
   def _get_jobs_metadata(self, job_ids: List[str]) -> List[Dict]:
      ...
      try:
         api_url = f"{Config.CANDIDATE_API_BASE_URL}/api/jobs/public/metadata"
   ...
   def _process_interaction_stream(self):
      ...
      try:
         ...
         messages = self.redis_client.xreadgroup(
            ...
            streams={stream_name: ">"},  # ">" = chỉ đọc messages chưa được deliver
            count=10,
   ...
   def _handle_interaction_event(self, fields: Dict[str, bytes]):
      ...
      try:
         ...
         if aggregate_type != "USER_INTERACTION":
            print(f"Skipping non-interaction event: aggregateType={aggregate_type}")
            return
         ...
         print(f"Processed interaction: user={user_id}, job={job_id}, type={interaction_type_upper}")
